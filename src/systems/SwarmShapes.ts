...
export function cubeShape(roomId: string, entitiesCount: number) {
  ...
  for (const [entity] of engine.getEntitiesWith(CansyComponent)) {
    ...
    const locTrans = C_TransformComponent.getMutable(entity).localTransform
...
export function randomShape(roomId: string, entitiesCount: number) {
...
export function matrixShape(roomId: string, entitiesCount: number) {
...
export function spiralShape(roomId: string, entitiesCount: number) {
...
export function goldenSpiralShape(roomId: string, entitiesCount: number) {
...
export function lissajousCurveShape(roomId: string, entitiesCount: number) {
  console.log('Shape Lissajous Curve, Room ID:', roomId)
  const substring = createSubstringExtractor(roomId)

  const parentScale = mapNumberToRange(parseInt(substring(2), 10), 0.01, 1)
  Transform.createOrReplace(artInstallation.parent(), {
...
export function mobiusStripShape(roomId: string, entitiesCount: number) {
...
export function torusKnotShape(roomId: string, entitiesCount: number) {
...
export function fibonacciSphereShape(roomId: string, entitiesCount: number) {
